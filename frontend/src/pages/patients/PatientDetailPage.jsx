...
      
      // Visits: Robust fetching logic
      try {
        const visitData = await visitService.listByPatient(id, token);
...
      setLoading(false);
    }
  }, [id, navigate, location.pathname, location.state, token, setError]); // ✅ ADDED setError
...
    try {
      await visitService.createVisit({ patient_id: patient.id, ...visitForm }, token);
      
