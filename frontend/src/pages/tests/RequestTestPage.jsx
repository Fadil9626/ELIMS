...
    Tag,
    ChevronDown,
    AlertTriangle, // For displaying URGENT priority
} from "lucide-react";
...
const RequestTestPage = () => {
    // 🔑 FIX 1: Use useSearchParams to read patientId and priority from the URL query string
    const [searchParams] = useSearchParams();
    const patientIdFromQuery = searchParams.get('patientId');
    const priorityFromQuery = searchParams.get('priority') || 'ROUTINE';
    
    // We use the ID from the path as a fallback, but the query ID is safer for this flow
    const { id: idFromPath } = useParams();
    const patientId = patientIdFromQuery || idFromPath;
...
                setLoading(true);
        
                // 🚀 Use patientId derived from path or query
                const [patientRes, catalogRes, deptsRes] = await Promise.all([
                    apiFetch(`/api/patients/${patientId}`),
                    apiFetch("/api/lab-config/tests/all"),
                    apiFetch("/api/departments").catch(() => []), // Fails gracefully
                ]);
        
...
        const map = {};
        for (const i of items) {
            const dept = (i?.department_name || item?.department || "Unassigned").toString();
...
            setSubmitting(true);
            
            // 🔑 FIX 2: Pass the priority to the API payload
            const response = await apiFetch("/api/test-requests", {
                method: 'POST',
                body: JSON.stringify({ 
                    patientId: Number(patientId), 
                    testIds: selectedItems,
                    priority: priorityFromQuery // 🔑 Include the derived priority here
                }),
            });
...
            
            // Navigate to the patient profile after successful order submission
...
        return <div className="text-center text-red-500">Patient not found.</div>;
    ...
    const isUrgent = priorityFromQuery.toUpperCase() === 'URGENT';
...
                    <div className={`bg-white p-5 rounded-xl shadow-lg border-l-4 ${isUrgent ? 'border-red-600' : 'border-blue-600'}`}>
...
                            Request Tests for {patient?.first_name} {patient?.last_name}
                            <span className={`px-3 py-1 rounded-full text-xs font-semibold border ${priorityClasses}`}>
                                {isUrgent && <AlertTriangle size={14} className="inline mr-1 animate-pulse" />}
                                {priorityFromQuery.toUpperCase()}
                            </span>
                        </h2>
                        <p className="text-sm text-gray-500 mt-1">
                            Patient ID: **{patient?.mrn || patient?.lab_id || patientId}**
                        </p>
                    </div>
