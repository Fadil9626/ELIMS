...
const AddEditStaffModal = ({
  ...
  onSave,
  userData,
  availableRoles = [], // <--- We will rely on this 100%
  ...
  useEffect(() => {
    ...
    if (isEditMode && userData) {
      // Extract Department ID reliably
      ...
      setFormData({
        ...
        password: "",
        // Ensure ID is string for select matching
  ...
  const loadPermissions = async () => {
    ...
    for (const id of formData.roleIds) {
      try {
        const response = await apiFetch(`/api/roles/${id}/permissions`);
        
        // ✅ FIX: Handle wrapped data { data: [...] } just in case
        const grants = Array.isArray(response) ? response : (response.data || []);
        ...
        perms[id] = [];
        console.error(`Failed to load permissions for role ${id}:`, e);
      }
    }
  ...
  // Run loadPermissions whenever roleIds change and view is open
  useEffect(() => {
    if (isOpen && permissionsView) { 
        loadPermissions();
    }
  ...
  // Helper to merge permission slugs
  ...
  return (
    <div className="fixed inset-0 bg-black/40 flex justify-center items-center z-50 backdrop-blur-sm">
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-lg p-6 ring-1 ring-gray-200 max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            ...
            {/* ✅ FIX: Use availableRoles directly. No more internal loading state needed. */}
            {availableRoles.length > 0 ? (
              ...
              <div className="text-sm text-rose-600 border border-rose-200 bg-rose-50 p-2 rounded">
                No roles available. Please ensure roles exist in RBAC or refresh the page.
              </div>
            )}
